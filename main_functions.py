...
the data frame if the shape is set to True. x is used to control the 
...
def upsample(A_B):
    ...
    numeric_data = A_B.select_dtypes(include=['number'])
    weekly = numeric_data.resample('W', label='left', closed='left').mean()
